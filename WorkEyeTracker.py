...
╔═══════════════════════════════════════════════════════════════╗
║                    WORK-EYE TRACKER                           ║
║              Pre-Configured for Shinde's Company               ║
╠═══════════════════════════════════════════════════════════════╣
║  Company ID: 12                                               ║
║  Token: ***QmE5cw==                                      ║
║  Generated: 2026-02-01 08:39:37 UTC                       ║
╠═══════════════════════════════════════════════════════════════╣
║  INSTRUCTIONS:                                                ║
    ...
    def fetch_configuration(self):
        ...
        try:
            ...
            if not CONFIG.get('tracker_token'):
                print("[CONFIG] No tracker token available")
                return False
            ...
            print(f"[CONFIG] Fetching configuration from {url}")
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    print(f"✅ Configuration synced successfully")
                    print(f"   📸 Screenshot interval: {screenshot_min} min ({CONFIG['screenshot_interval']}s)")
                    print(f"   ⏰ Idle threshold: {idle_min} min ({CONFIG['idle_threshold']}s)")
                    print(f"   🏢 Office hours: {CONFIG['office_start_time']} - {CONFIG['office_end_time']}")
                    print(f"   📅 Working days: {CONFIG['working_days']}")
                    
                    return True
                else:
                    print(f"[CONFIG] Server returned error: {data}")
            else:
                print(f"[CONFIG] HTTP {response.status_code}: {response.text}")
                
        except requests.exceptions.Timeout:
            print("[CONFIG] Request timeout - server may be slow")
        except requests.exceptions.ConnectionError:
            print("[CONFIG] Connection error - check network and backend URL")
...
def capture_screenshot():
    try:
        ...
        # Save a local copy under %APPDATA%/Tracker/screenshots
        try:
            ...
            print(f"[SCREENSHOT] Saved local copy: {filepath}")
        except Exception:
...
def verify_member(email):
    ...
    try:
        ...
        if not CONFIG.get('tracker_token'):
            print("[VERIFY] No tracker token configured")
        ...
        print(f"[VERIFY] Sending to: {url}")
        print(f"[VERIFY] Device ID: {STATE.device_id}")
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                print(f"[VERIFY] ✅ Verified: {member.get('name', 'Unknown')}")
                print(f"[VERIFY] ✅ Member ID: {member.get('id')}")
                print(f"[VERIFY] ✅ Device registered in DB")
            ...
            try:
                ...
                print(f"[VERIFY] ❌ HTTP {response.status_code}: {error}")
                return False, None, error
            except:
                print(f"[VERIFY] ❌ HTTP {response.status_code}")
        ...
        print("[VERIFY] ❌ Connection failed")
...
def punch_in():
    ...
    try:
        ...
        print(f"[PUNCH-IN] Email: {CONFIG['member_email']}")
        ...
        print(f"[PUNCH-IN] Response: {response.status_code}")
        
        if response.status_code == 200:
            data = response.json()
            print(f"[PUNCH-IN] ✅ {data.get('message')}")
            print(f"[PUNCH-IN] ✅ Punch log ID: {data.get('punchlogid')}")
            return True, data.get('message', 'Punched in successfully')
        else:
            try:
                error = response.json().get('error', 'Punch in failed')
                print(f"[PUNCH-IN] ❌ {error}")
                return False, error
            except:
...
def punch_out():
    ...
    try:
        ...
        print(f"[PUNCH-OUT] Email: {CONFIG['member_email']}")
        ...
        print(f"[PUNCH-OUT] Response: {response.status_code}")
        
        if response.status_code == 200:
            data = response.json()
            print(f"[PUNCH-OUT] ✅ {data.get('message')}")
            print(f"[PUNCH-OUT] ✅ Duration: {data.get('workdurationseconds', 0)}s")
            return True, data.get('message', 'Punched out successfully')
        else:
            try:
                error = response.json().get('error', 'Punch out failed')
                print(f"[PUNCH-OUT] ❌ {error}")
                return False, error
            except:
                return False, f"HTTP {response.status_code}"
...
def upload_data():
    ...
    try:
        ...
        if response.status_code == 200:
            print("[UPLOAD] ✅ Data uploaded")
            STATE.reset_for_upload()
            return True
        else:
            print(f"[UPLOAD] ⚠️ Failed: {response.status_code}")
            return False
...
def heartbeat():
    try:
        ...
        payload = {
            ...
            'osinfo': STATE.os_info,
            'isidle': STATE.is_idle,
            'idlefor': STATE.idle_for
        }

        print("[HEARTBEAT DEBUG]", payload)
        ...
        if response.status_code == 200:
            print("[HEARTBEAT] ❤️ Sent successfully")
...
class ActivityTracker(Thread):
    ...
    def run(self):
        print("[ACTIVITY] Activity tracker started")
        ...
        while self.running:
            ...
            print(f"[IDLE DEBUG] IdleTime={idle_time:.1f}s | Threshold={CONFIG['idle_threshold']} | Idle={idle}")
...
class ConfigurationFetcher(Thread):
    ...
    def run(self):
        print("[CONFIG-THREAD] Configuration fetcher started")
        
        # Initial fetch after 5 seconds (give time for tracking to start)
        time.sleep(5)
        if STATE.is_tracking:
            print("[CONFIG-THREAD] Performing initial configuration fetch")
            config_manager.fetch_configuration()
        ...
        while self.running:
            ...
            if STATE.is_tracking and config_manager.should_sync():
                print("[CONFIG-THREAD] Configuration sync cycle triggered")
                config_manager.fetch_configuration()
...
class ScreenshotCapture(Thread):
    ...
    def run(self):
        print("[SCREENSHOT-THREAD] Screenshot capture thread started")
        ...
        while self.running:
            ...
            try:
                ...
                if screenshot_b64:
                    ...
                    print(f"[SCREENSHOT] 📸 Captured at {datetime.now().strftime('%H:%M:%S')} (interval: {interval_min}min)")
            except Exception as e:
            ...
            while elapsed < interval and self.running and STATE.is_tracking:
                ...
                if elapsed < interval and CONFIG['screenshot_interval'] != interval:
                    print(f"[SCREENSHOT] ⚙️ Interval changed during wait, adjusting...")
                    break
...
class MinimalUI:
        ...
        try:
            ...
            if result:
                ...
                if punch_in():
                    ...
                    print("[UI] Starting monitoring threads...")
                    ...
                    print(f"📸 Screenshot interval: {CONFIG['screenshot_interval']}s ({CONFIG['screenshot_interval']//60}min)")
                    print(f"⏰ Idle threshold: {CONFIG['idle_threshold']}s ({CONFIG['idle_threshold']//60}min)")
                    print(f"🔄 Config sync: Every 5 minutes")
                    print("=" * 70)
                else:
    ...
    def start_tracking(self):
        ...
        print(f"✅ Tracking: {CONFIG['member_email']} (Device: {STATE.device_id})")
        print(f"✅ Screenshot interval: {CONFIG['screenshot_interval']}s")
        print(f"✅ Idle threshold: {CONFIG['idle_threshold']}s")
...
if __name__ == '__main__':
    print("="*70)
    print("WORK TRACKER - Synced with Backend DB")
    print("="*70)
    print(f"Device ID: {STATE.device_id} (Auto-generated)")
